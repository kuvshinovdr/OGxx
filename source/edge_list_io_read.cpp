...
namespace ogxx::io
{
  auto read(string_view& in, ogxx::Edge_list& el, Edge_list_format const& format)
    -> bool
  {
    if (!util::accept_tokens(in, format.list_open)) return false;

    std::vector<Vertex_pair> data;

    for (; !util::accept_tokens(in, format.list_close);)
    {
      util::accept_tokens(in, format.pair_open);
      ...
      do
      {
        ...
        temp.push_back(t);
      } while (util::accept_tokens(in, format.pair_sep));
      ...
      util::accept_tokens(in, format.pair_close);
    }
